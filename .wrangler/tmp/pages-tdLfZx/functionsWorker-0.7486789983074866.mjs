...
var ALLOWED_HOSTS = [
  "123.140.197.51",
  // 주요 관광지 CCTV (Port 80) - 제주공항, 성산일출봉, 새연교
  "59.8.86.94",
  // 방재 시스템 CCTV (Port 8080 → 80 우회 불가, 차단됨)
  "211.114.96.121",
  // 제주시 해수욕장 CCTV (Port 1935 → 80 우회 불가, 차단됨)
  "211.34.191.215",
  // 서귀포 CCTV
  "hallacctv.kr",
  // 한라산 CCTV (CORS 지원, 직접 접근 가능)
  "www.hallacctv.kr"
];
async function onRequest(context) {
  ...
  try {
    ...
    const isM3u8 = contentType.includes("application/vnd.apple.mpegurl") || contentType.includes("application/x-mpegurl") || targetUrlStr.toLowerCase().includes(".m3u8");
    const corsHeaders = {
      "Access-Control-Allow-Origin": "*",
      "Access-Control-Allow-Methods": "GET, OPTIONS",
      "Access-Control-Allow-Headers": "*",
...
async function onRequest3(context) {
  ...
  try {
    ...
    const decodeHtmlEntities = /* @__PURE__ */ __name((str) => str.replace(/&#(\d+);/g, (match3, dec) => String.fromCharCode(dec)), "decodeHtmlEntities");
    const stripTags = /* @__PURE__ */ __name((str) => decodeHtmlEntities((str || "").replace(/<[^>]*>?/gm, "").replace(/&nbsp;/g, "").replace(/\s+/g, " ").trim()), "stripTags");
    const results = [];
    let match2;
    while ((match2 = blockPattern.exec(html)) !== null) {
    ...
    if (hostname.includes("apis.data.go.kr") || hostname.includes("openapi.airport.co.kr") || hostname.includes("api.visitjeju.net") || hostname.includes("api.jejuits.go.kr")) {
      ...
      if (serviceKey) {
        ...
        if (!targetUrl.searchParams.has(keyParam)) {
          ...
          try {
            targetUrl.searchParams.set(keyParam, decodeURIComponent(rawKey));
          } catch (e) {
    ...
    if (contentType.includes("application/vnd.apple.mpegurl") || targetUrlString.toLowerCase().includes(".m3u8")) {
      ...
      const rewrittenLines = sourceText.split("\n").map((line) => {
        ...
        if (t.length > 0 && !t.startsWith("#")) {
          try {
            return `${proxyBase}${encodeURIComponent(new URL(t, targetUrlString).href)}`;
          } catch (e) {
            return `${proxyBase}${encodeURIComponent(targetUrlString.substring(0, targetUrlString.lastIndexOf("/") + 1) + t)}`;
          }
        }
        return line;
      });
...
async function onRequest10(context) {
  ...
  const ALLOWED_DOMAINS = [
    ...
    // CCTV 스트리밍 서버 IP (구)
    "211.114.96.121",
    // CCTV 스트리밍 서버 IP (제주시 1권역)
    "211.34.191.215",
    // CCTV 스트리밍 서버 IP (서귀포/동부/서부)
    "59.8.86.15",
    // CCTV 스트리밍 서버 IP (서귀포 일부)
    "119.65.216.155",
    // 한라산 CCTV 스트리밍 서버 IP
    "hallacctv.kr",
  ...
  if (pathname === "/api/public-data" || url.searchParams.has("url")) {
    ...
    try {
      ...
      const isJsonExpected = url.searchParams.get("dataType") === "JSON" || targetUrl.searchParams.get("dataType") === "JSON" || targetUrl.searchParams.get("_type") === "json" || targetUrlString.toLowerCase().includes("datatype=json") || targetUrlString.toLowerCase().includes("returntype=json") || targetUrlString.toLowerCase().includes("_type=json");
      ...
      for (const [key, value] of response.headers.entries()) {
        ...
        if (lowerKey !== "content-encoding" && lowerKey !== "content-length" && lowerKey !== "transfer-encoding" && lowerKey !== "content-security-policy") {
          newHeaders.set(key, value);
        }
      }
      const contentType = response.headers.get("content-type") || "";
      const isM3U8Response = contentType.includes("application/vnd.apple.mpegurl") || contentType.includes("audio/mpegurl") || targetUrlString.toLowerCase().includes(".m3u8");
      if (isM3U8Response) {
...
// ../.wrangler/tmp/pages-tdLfZx/functionsRoutes-0.34861593492056386.mjs
var routes = [
  {
...
// ../../Users/k9746/AppData/Roaming/npm/node_modules/wrangler/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// ../../Users/k9746/AppData/Roaming/npm/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
...
// ../../Users/k9746/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../Users/k9746/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// ../.wrangler/tmp/bundle-tCKuwh/middleware-insertion-facade.js
...
// ../../Users/k9746/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// ../.wrangler/tmp/bundle-tCKuwh/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7486789983074866.mjs.map
