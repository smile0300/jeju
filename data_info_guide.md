...
---
---
## 9. API 연동 트러블슈팅 및 레슨 런 (Troubleshooting & Lessons Learned)
최근 운영 중 발생한 이슈와 그 해결 과정을 향후 유지보수를 위해 기록합니다.

### 9.1. 기상청 API 파라미터 누락 이슈
- **증상**: 날씨 정보가 업데이트되지 않고 고정된 데이터만 표시됨.
- **원인**: Cloudflare Worker 프록시가 `nx`, `ny` 등 쿼리 파라미터를 대상 서버로 전달하지 않고 차단함.
- **해결**: Worker 코드를 수정하여 `endpoint` 파라미터 외의 모든 쿼리 스트링을 대상 URL에 동적으로 병합하도록 개선함.

### 9.2. 공항공사 API 417 Expectation Failed 에러
- **증상**: 항공편 조회 시 417 에러와 함께 데이터 로드 실패.
- **원인**: 브라우저가 자동 생성하는 `Expect`, `Host`, `Origin` 등의 헤더가 프록시를 통해 공항공사 서버로 전달되었으나, 해당 서버가 이를 처리하지 못해 발생.
- **해결**: 
    1. **헤더 세정**: Worker에서 `fetch` 요청 시 브라우저 헤더를 그대로 넘기지 않고, `Accept`, `User-Agent` 등 최소한의 필수 헤더만 선별적으로 전달하도록 수정.
    2. **프로토콜 조정**: 특정 환경에서 HTTPS 호출 시 발생하는 `NO OPENAPI SERVICE ERROR` 대응을 위해 엔드포인트를 HTTP로 유지하되 데이터 무결성을 보장함.

### 9.3. 실시간 한라산 상태 파싱
- **증상**: 제주도청 웹사이트 구조 변경 시 파싱 실패 가능성.
- **방어 로직**: 정규표현식을 이용한 데이터 추출이 실패할 경우, 사용자에게 친절한 에러 메시지와 함께 공식 사이트 직결 링크를 제공하도록 예외 처리 강화.
